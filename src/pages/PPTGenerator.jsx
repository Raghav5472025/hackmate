...
export default function PPTGenerator() {
  ...
  const [showOptions, setShowOptions] = useState(false)  // ← CHANGE 1: options hidden by default
  ...
  async function handleGenerate() {
    ...
    try {
      // CHANGE 2: agar options nahi dikhaye to sirf topic bhejo
      const fullTopic = !showOptions
        ? topic
        : layout === 'custom' && customDesc
          ? `${topic}. ${customDesc}`
  ...
  function SlidePreview({ slide, index, theme, onClick, isEditing }) {
    ...
    return (
      <div onClick={onClick} style={{
        ...
        transition: 'all 0.15s', minHeight: 120, position: 'relative',
      }}
        onMouseEnter={e => !isEditing && (e.currentTarget.style.transform = 'translateY(-2px)')}
        onMouseLeave={e => e.currentTarget.style.transform = ''}
      >
        <div style={{ position: 'absolute', top: 8, left: 8, width: 20, height: 20, borderRadius: '50%', background: isTitle ? 'rgba(255,255,255,0.2)' : T.primary, color: 'white', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {index + 1}
        </div>
        <div style={{ paddingTop: 24 }}>
          {slide.emoji && <div style={{ fontSize: 20, marginBottom: 4 }}>{slide.emoji}</div>}
          <p style={{ fontSize: 12, fontWeight: 800, color: isTitle ? 'white' : '#1a1a2e', marginBottom: 4, lineHeight: 1.2 }}>{slide.title}</p>
      ...
      <div className="container" style={{ maxWidth: 1000 }}>
          ...
          <div className="fade-up" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 280px', gap: '1.5rem', alignItems: 'start' }}>
            ...
            <div className="card" style={{ padding: '1.75rem' }}>
              <div className="form-group" style={{ marginBottom: '1.5rem' }}>
                ...
                />
                <p style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 4 }}>Ctrl+Enter to generate directly</p>
              </div>
              ...
              {/* CHANGE 3: Direct generate button — big & primary */}
              <button className="btn btn-primary btn-block btn-lg" onClick={handleGenerate} disabled={!topic.trim()} style={{ gap: 8, marginBottom: '1rem' }}>
                ⚡ Generate PPT Directly →
              </button>

              {/* Optional customize toggle */}
              <button
              ...
              >
                {showOptions ? '▲ Hide options' : '⚙️ Customize layout & theme — optional'}
              </button>

              {/* Options panel — only when toggled */}
              {showOptions && (
              ...
              <p style={{ fontSize: 12, color: 'var(--text-3)', textAlign: 'center', marginTop: 12 }}>Takes ~10 seconds · 9 slides · .pptx download</p>
            </div>

            {/* Right — theme selector */}
            <div>
              <div className="card" style={{ padding: '1.25rem', marginBottom: '1rem' }}>
                <p className="form-label" style={{ marginBottom: 12 }}>Color theme</p>
        ...
        {step === 3 && slides.length > 0 && (
          <div className="fade-up">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem', flexWrap: 'wrap', gap: 10 }}>
              <div>
                <h2 style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-1)', marginBottom: 4 }}>{slides.length} slides ready — review & edit</h2>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button className="btn btn-primary" onClick={handleCreate} style={{ gap: 6 }}>⬇️ Download PPT →</button>
              </div>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: 10, marginBottom: '1.25rem' }}>
              {slides.map((slide, i) => (
                <SlidePreview key={i} slide={slide} index={i} theme={theme}
            ...
            {editingSlide !== null && slides[editingSlide] && (
              <div className="card fade-up" style={{ padding: '1.5rem', marginBottom: '1.25rem', borderColor: selectedTheme?.primary + '40' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                  <p style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-1)' }}>Editing Slide {editingSlide + 1}</p>
            ...
            {error && <div style={{ fontSize: 13, color: 'var(--red)', background: 'var(--red-bg)', padding: '10px 14px', borderRadius: 8, marginBottom: '1rem', border: '1px solid var(--red-border)' }}>❌ {error}</div>}
            <button className="btn btn-primary btn-block btn-lg" onClick={handleCreate} style={{ gap: 8 }}>⬇️ Download .pptx →</button>
          </div>
        )}
        ...
        {step === 4 && result && (
          <div className="card fade-up" style={{ padding: '2.5rem', textAlign: 'center', background: 'linear-gradient(135deg,#f0fdf4,white)', borderColor: 'rgba(22,163,74,0.3)' }}>
            ...
            <h2 style={{ fontSize: 26, fontWeight: 800, color: 'var(--text-1)', marginBottom: 8 }}>Downloaded!</h2>
            <p style={{ fontSize: 15, color: 'var(--text-2)', marginBottom: '2rem' }}>{result.slideCount} slides — check your Downloads folder</p>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
              <button className="btn btn-primary" onClick={handleCreate}>⬇️ Download Again</button>
              <button className="btn btn-secondary btn-sm" onClick={() => { setStep(1); setSlides([]); setResult(null); setTopic('') }}>+ New Presentation</button>
              <button className="btn btn-ghost btn-sm" onClick={() => setStep(3)}>← Edit Slides</button>
            </div>
          </div>
        )}
      </div>
...
function generateFallbackSlides(topic, profile) {
  return [
    ...
    { layout: 'tech',    title: 'Tech Stack', content: [`Frontend: React / React Native`, `Backend: ${profile?.skills?.includes('Python') ? 'FastAPI + Python' : 'Node.js'}`, 'AI/ML: Claude / Gemini', 'Database: PostgreSQL', 'Cloud: Vercel'], emoji: '🛠️' },
    ...
    { layout: 'closing', title: 'Thank You', subtitle: "Let's build the future together", emoji: '🎯' },
  ]
}